...
def choice_char_class():
    ...
    while approve_choice != 'y':
        char_class = input(',: Воин — warrior, Маг — mage, Лекарь — healer:')
        if char_class == 'warrior':
            print('Воитель — воин ближнего боя. Сильный, выносливый .')
        if char_class == 'mage':
            print('Маг — воин дальнего боя..')
        if char_class == 'healer':
            print('Лекарь — заклинатель. Черпает силы из , веры и духов.')
        approve_choice = input('(Y), ').lower()
    return char_class
